...
#from written by Jeremy M. Beaulieu
...
check_bounds <- function(params, lower_bounds, upper_bounds, verbose=FALSE){
    ...
    if (any(below_lower)) {
        if (verbose) {
            ...
            cat("  Adjusted to: ", lower_bounds[below_lower], "\n\n")
        }
    ...
    if (any(above_upper)) {
        if (verbose) {
            ...
            cat("  Adjusted to: ", upper_bounds[above_upper], "\n\n")
        }
...
### Make into a new preproces function!!!! 
if (reg == 'BM1') {get.root.theta <- TRUE } else {get.root.theta <- FALSE}

            tree_info <- preprocessTree(inputs=idata, reg = reg, scaleHeight=FALSE, root.fixed=ss[i, 6], root.age=NULL)
            ...
            defaults$weight_type <- 'alt'
            defaults$algorithm <- 'three.point'
