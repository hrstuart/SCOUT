.libPaths("/dartfs/rc/lab/M/McKennaLab/projects/hannah/software/R/R-4.4.2/library")
...
source('/dartfs/rc/lab/M/McKennaLab/projects/hannah/utils/SCOUT/SCOUT-em/SCOUT_em_setup.R') 
source('/dartfs/rc/lab/M/McKennaLab/projects/hannah/utils/SCOUT/V2/SCOUT_EM_V4_TMP_RUNNER.R') ## SWITCHING TO TMP WHILE TROUBLESHOOTING 
source('/dartfs/rc/lab/M/McKennaLab/projects/hannah/utils/SCOUT/V2/SCOUT_EM_V4_utilities.R')
...
for (i in 1:nrow(ss)){
    ...
    filename <- sprintf('%s/%s_%s.rds', wrkdir, ss[i, 'testid'], format(Sys.Date(), "%Y%m%d"))
    saveRDS(final_list, filename)

    log_message(sprintf('Done with %s || %s / %s \n', ss[i, 'testid'], as.character(i), as.character(nrow(ss))) , logfile, verbose=TRUE)
    log_message('=========================================================\n', logfile, verbose=TRUE)
